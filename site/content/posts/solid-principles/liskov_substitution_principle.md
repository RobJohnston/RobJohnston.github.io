+++
title = "Interchangeable Components: When Inheritance Works (and When It Doesn't)"
date = 2026-01-19
...
image = "/images/solid/temperature_sensor_fast-001.png"
...
class UncalibratedSensor(TemperatureSensor):
    ...
    def calibrate(self, reference_temp: float, measured_temp: float):
        ...
        This is better than silently doing nothing - the caller knows
        immediately that calibration isn't supported.
        """
        raise NotImplementedError(
...
`UncalibratedSensor` raises an error for calibration rather than silently failing. This is better than violating expectations.

### 4. Range Restrictions Are Clear

`FastTemperatureSensor` documents its narrower range. The base class allows subclasses to be more restrictive (narrower range) but not less restrictive (wider range).
...
When you violate LSP, you create **silent failures**:

**Temperature monitoring system**:

- Student A's code works with PrecisionSensor
- Student B swaps in CheapSensor (subclass, should work!)
- Results are silently wrong (Fahrenheit treated as Celsius)
- Paper is published with flawed data
- Months later, the error is discovered

**The problem**: The code didn't **fail**, it **lied**. If CheapSensor couldn't pretend to be a TemperatureSensor, the error would have been caught immediately.

**Better approach**: Either honour the contract (convert F→C internally) or don't inherit (use a different type entirely).

## Common LSP Violations in Scientific Code

Here are four patterns that frequently violate LSP in scientific codebases, along with how to fix them:

### Violation 1: Changing Units or Conventions
...
# VIOLATION: Subclass silently returns radians
class RadianAngle(Angle):
    def to_degrees(self) -> float:
        """Actually returns radians!"""
        return self._rad  # ❌ Caller expects degrees

```

**Fix**: Keep the same units, or don't use inheritance.

### Violation 2: Throwing Unexpected Exceptions

```python
class DataLoader:
    def load(self, filename: str) -> np.ndarray:
        """Load data. Raises IOError if file not found."""
        pass

# VIOLATION: New exception not in base class contract
class DatabaseLoader(DataLoader):
    def load(self, filename: str) -> np.ndarray:
        """Load from database."""
        if not self.connection_established:
            raise ConnectionError("Database offline")  # ❌ Unexpected!
```

**Fix**: Only raise exceptions the base class promises.

### Violation 3: Returning Incompatible Types

```python
class Spectrometer:
    def measure(self) -> Spectrum:
        """Return spectrum object."""
        return Spectrum(wavelengths, intensities)

# VIOLATION: Changes return type structure
class MultiChannelSpectrometer(Spectrometer):
    def measure(self) -> list[Spectrum]:  # ❌ Different return type!
        """Return list of spectra."""
        return [Spectrum(...), Spectrum(...)]
```

**Fix**: Maintain the same return type structure.

### Violation 4: Strengthening Preconditions

```python
class SignalProcessor:
    def process(self, data: np.ndarray):
        """Process any 1D array."""
        pass

# VIOLATION: More restrictive input requirements
class FFTProcessor(SignalProcessor):  # Violates LSP!
    def process(self, data: np.ndarray):
        """Process signal. Data length must be power of 2."""  # ❌ Stronger requirement!
        if len(data) & (len(data) - 1) != 0:
            raise ValueError("Length must be power of 2")
```

**Fix**: Handle non-power-of-2 lengths internally (pad if needed).
